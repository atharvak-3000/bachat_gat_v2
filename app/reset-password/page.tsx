...
function ResetPasswordForm() {
  ...
  if (!token) {
    return (
      <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/30 text-red-800 dark:text-red-400 p-6 rounded-2xl text-sm font-semibold space-y-4">
        <p>⚠️ Missing or invalid password reset token.</p>
        <Link
          href="/forgot-password"
          className="inline-block bg-red-600 text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-red-700 transition"
        >
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm font-medium leading-relaxed">
          Please create a secure password of at least 8 characters for your Bachatgat account.
        </p>
      </div>

      {success ? (
        <div className="bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-900/30 text-green-800 dark:text-green-400 p-5 rounded-2xl text-sm font-bold leading-relaxed animate-fadeIn space-y-2">
          <p>🎉 Password successfully updated!</p>
          <p className="text-xs text-gray-500 font-normal">Redirecting you to the sign-in page...</p>
        </div>
      ) : (
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-orange-50 to-amber-50 dark:from-[#0D1021] dark:to-[#0F1117] flex items-center justify-center p-4 transition-colors duration-250">
      <div className="w-full max-w-md">
        <div className="bg-white dark:bg-[#1A1D27] rounded-3xl border border-gray-200 dark:border-gray-800 shadow-xl p-8">
          <Suspense fallback={<div className="text-center p-4 text-sm text-gray-500">Loading token...</div>}>
            <ResetPasswordForm />
          </Suspense>
        </div>
      </div>
