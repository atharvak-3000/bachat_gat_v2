...
import { createAuthToken, verifyAuthToken, toSafeMember } from '../lib/auth'
...
async function runSeedAndTest() {
  ...
  const org = await prisma.organization.create({
    data: {
      ...
      district: 'Pune',
      groupCode: 'SG001',
      monthlySavingAmount: BigInt(20000), // ₹200 in paise
  ...
  const superadminLoginResponseBody = JSON.stringify(
    {
      success: true,
      member: toSafeMember(superadmin),
    },
    null,
    2
  )
  ...
  console.log('\ngetCurrentMember() for SUPERADMIN resolved (safe member):')
  console.log(JSON.stringify(toSafeMember(resolvedSuperadmin), null, 2))
  ...
  console.log('\ngetCurrentMember() for MEMBER resolved (safe member):')
  console.log(JSON.stringify(toSafeMember(resolvedMember), null, 2))
